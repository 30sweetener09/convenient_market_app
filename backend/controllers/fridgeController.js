...
export const createFridgeItem = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error", resultCode: "00197" });
  }
};
...
export const updateFridgeItem = async (req, res) => {
  try {
    ...
    if (note !== undefined) updates.note = note; // If no real updates were made (only updated_at), although we checked this before, safer to re-check
